...
DROP
DATABASE IF EXISTS megacity_test_db;

-- Create the Database
CREATE
DATABASE IF NOT EXISTS megacity_test_db;
...
BEGIN
SELECT
    YEAR (createdAt) AS period, SUM (total) AS totalRevenue, SUM (discount) AS totalDiscounts, SUM (tax) AS totalTaxes, SUM (total - discount + tax) AS netRevenue
FROM booking
WHERE status = 'confirmed'
